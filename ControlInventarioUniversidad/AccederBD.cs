    ...
    class AccederBD
    {
        ...
        public bool ConectaDB() //inicia conexión a la BD
        {
            ...
            try
            {
                ...
                //con = new MySqlConnection("Server = 127.0.0.1;Database=inventariouniversidad;Uid=root;Pwd=moscaslibres");  //offline
                con.Open();
                res = true;
            }
            catch (MySqlException mse)
            {
                AccederBD.Error = "Error SQL al conectar. " + mse.Message;
            }
            catch (Exception general)
            {
                AccederBD.Error = "Error general al conectar. " + general.Message;
            }
            return res;
        } //final conectar DB
        ...
        public bool login(string usuario, string pass) //Verifica acceso y mueve al form correcto
        {
            ...
            try
            {
                string query = "SELECT USER_TYPE FROM inventory_usuario WHERE USER_name = '" + usuario + "'and PASSWORD = '" + pass + "'";
                com = new MySqlCommand();   //conexión arreglada inicio
                com.CommandText = query;
                ConectaDB();
                com.Connection = this.con;
                com.ExecuteNonQuery();      //conexión arreglada fin
                valor = int.Parse(com.ExecuteScalar().ToString());
                res = true;
                if (valor == 0)
                {
